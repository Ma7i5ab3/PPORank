...
def tcga_results(brca_df, pred_prefix, test_ids, lambdas, gamma, k, nfolds, verbose=True):
    # test_ids like  ['TCGA-3C-AAAU', 'TCGA-3C-AALI'],
    # TCGA samples:['TCGA-C8-A12Z', 'TCGA-B6-A408']
    ...
    chr17_pos = np.array([~np.isnan(x) and x >= 2 for x in np.array(
        brca_df.loc[:, CHR17])[brca_keep_index]])
    er_neg = np.array(brca_df.loc[:, ER])[brca_keep_index] == NEGATIVE
    pr_neg = np.array(brca_df.loc[:, PR])[brca_keep_index] == NEGATIVE
    mbrca_pos = np.array(brca_df.loc[:, BRCA_GERM])[brca_keep_index] == True  # 37, mBRCA mutant
    jak2_pos = np.array([~np.isnan(x) and x >= 0 for x in np.array(
        brca_df.loc[:, JAK2_RPPA])[brca_keep_index]])
    # estrogen receptor (ER), progesterone receptor (PR) and without
    # alteration of the human epidermal growth factor receptor type 2 (HER2) gene
    tnbc = her2_neg & ~chr17_pos & er_neg & pr_neg  # three negative, herb negative,chr17
    tnbc_mbrca_pos = tnbc & mbrca_pos  # 9
    tnbc_jak2_pos = tnbc & jak2_pos  # 56
    ...
    for Lambda in lambdas:
        if len(lambdas) > 1:
            ...
            for i in range(nfolds):
                fold = np.load('{}fold{}_k{}_l{}_g{}.npz'.format(
                    pred_prefix, i, k, Lambda, gamma))
                ndcgs.extend(NDCG(fold['true_Y'], fold['pred_Y'], k))
            ndcgs = np.array(ndcgs)
            ndcg = np.mean(ndcgs[~np.isnan(ndcgs)])
            if verbose:
                print('CV, NDCG@{} = {:.4f}, lambda = {}, gamma = {}'.format(
                    k, ndcg, Lambda, gamma))
        else:
            if verbose:
                print(
                    'Skipping cross-validation because there is only one lambda to choose from.')
            ndcg = None
    ...
    pred_fn = '{}pred_k{}_l{}_g{}.npz'.format(
        pred_prefix, k, best_lambda, gamma)  # 'PPO_TCGA_BRCA_pred_k10_l1.2_g1.8.npz'
    pred = np.load(pred_fn)
    test_Y_pred = pred['test_Y_pred']
    test_Y_pred = test_Y_pred[test_keep_index]  # （1080，265）

    drug_indexes = {}
    if 'drug_names' in pred:
        drug_names = np.array([x.lower() for x in pred['drug_names']])
        for drug in [LAPATINIB] + TNBC_DRUGS:
            drug_indexes[drug] = np.nonzero(
                drug_names == DRUG_NAMES[drug].lower())[0][0]
    else:
    ...
    for drug in TNBC_DRUGS:
        ...
        lap_wins[drug] = test_Y_pred[:,
                                     lapatinib_index] > test_Y_pred[:, drug_index]
    ...
    if verbose and 'drug_names' not in pred:
        ...
        print(
            '! Assuming that lapatinib is at position 0 and TNBC drugs follow in a pre-specified order in pred[\'test_pred_Y\'] !')
    ...
    if verbose and len(lambdas) > 1:
        print('BEST CV, NDCG@{} = {:.4f}, lambda = {}, gamma = {}'.format(
            k, best_ndcg, best_lambda, gamma))
    ...
    FULL_LINE = '------------------------------------------------------------------'
    print('\n', FULL_LINE)
    print('| {:<23} | {:<10} | {:<10} | {:<10} |'.format(
        'Recommendation', 'HER2+', 'TNBC mBRCA', 'TNBC JAK2+'))
    print(FULL_LINE)
    for drug in [PARPi] + PARP_INHIBS + [RUXOLITINIB]:
        if drug == RUXOLITINIB:
            print('| lapatinib > {:<11} | {:<10} | {:<10} | {:<10} |'.format(
                drug, '{:.2f}'.format(acc_her2_pos[drug]),
                      '---', '{:.2f}'.format(1 - acc_tnbc_jak2_pos[drug])))
        else:
            print('| lapatinib > {:<11} | {:<10} | {:<10} | {:<10} |'.format(drug, '{:.2f}'.format(
                acc_her2_pos[drug]), '{:.2f}'.format(1 - acc_tnbc_mbrca_pos[drug]), '---'))
    print(FULL_LINE)
