...
**RESULT (2026-06-13) — hypothesis REJECTED.** EN-on-kernel mean = **0.780** (per-fold
0.7745 / 0.7753 / 0.7770 / 0.7928 / 0.7782; alpha 0.01–0.1, l1_ratio 0.5). Essentially
identical to EN-on-genes (0.778). So EN's strength is **NOT** a feature-representation
artifact — EN is a genuinely strong baseline on GDSC full-rank NDCG (consistent with the
literature: per-drug linear regression is very competitive on drug response; full-rank NDCG
is also compressed upward). Reframing: the open question is no longer "why is EN too high"
but **"is our PPORank/CaDRReS too low vs the paper?"** — the paper may well have EN≈0.78 with
PPORank marginally above. Verdict pending the fixed PPORank run: if it climbs to ~0.78–0.80
the comparison reconciles; if it stays ~0.71, PPORank is the real underperformer (dig deeper
than features+hyperparameters). EN stays on the kernel (paper-faithful, fair comparison) regardless.
...
| **EN ≈ 0.78 — feature hypothesis rejected** | reframed → see "PPO too low?" | EN-on-kernel (0.780) ≈ EN-on-genes (0.778): EN's strength is NOT a feature artifact, it's a genuinely strong baseline. EN kept on kernel (paper-faithful) + l1_ratio=0.5. Real question moved to whether fixed PPORank/CaDRReS reach ~0.78 |
